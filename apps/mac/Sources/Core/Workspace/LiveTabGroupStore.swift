...
final class LiveTabGroupStore: ObservableObject {
    ...
    func setGuideVisible(_ visible: Bool, id: String) {
        guard let index = groups.firstIndex(where: { $0.id == id }) else { return }
        groups[index].isGuideVisible = visible
        activeGroupID = id
    }

    func toggleGuideVisibility(id: String) {
        guard let index = groups.firstIndex(where: { $0.id == id }) else { return }
        groups[index].isGuideVisible.toggle()
        activeGroupID = id
    }
    ...
    func expand(id: String? = nil) {
        ...
        WindowTiler.batchRaiseAndDistribute(
            ...
            region: groups[index].placement.fractions,
        ...
        [
            ...
            "groups": groups.map { group in
                [
                    ...
                    "guideVisible": group.isGuideVisible,
