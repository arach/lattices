...
struct HUDLiveTabChrome: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 0) {
            ...
            .background(HUDLiveTabDragRegion())
            .help("Drag to move these tab tools")
            ...
            HStack(spacing: 2) {
                modeButton
                Button {
                    store.setGuideVisible(false, id: group.id)
                } label: {
                ...
                .buttonStyle(.plain)
                .help("Hide tab tools — restore them from Groupings in Hyperspace")
                .accessibilityLabel("Hide tab tools for \(group.name)")
                ...
                Button {
                    ...
                    Image(systemName: "minus.circle")
                        .font(.system(size: 9, weight: .semibold))
                        .foregroundStyle(Palette.kill.opacity(0.62))
                        .frame(width: 27, height: 27)
                        .contentShape(Rectangle())
                }
                .buttonStyle(.plain)
    ...
    private var modeButton: some View {
        ...
        .accessibilityLabel(group.isExpanded ? "Return to tabs" : "Split tab group")
    }
...
private struct HUDLiveTabDragRegion: NSViewRepresentable {
    func makeNSView(context: Context) -> NSView {
        HUDLiveTabDragRegionView()
    }

    func updateNSView(_ nsView: NSView, context: Context) {}
}

private final class HUDLiveTabDragRegionView: NSView {
...
final class HUDLiveTabChromes: NSObject, NSWindowDelegate {
    ...
    private var groupIDByPanel: [ObjectIdentifier: String] = [:]
    private var manualOrigins: [String: NSPoint] = [:]
    ...
    func update(groups: [LiveTabGroup], windows: [UInt32: WindowEntry], obscuredBy obscured: [NSRect]) {
        ...
        for group in groups {
            ...
            // Leave a dedicated action gutter so the grid and ungroup tools
            // never crowd or clip the final tab in the rail.
            let preferredWidth = CGFloat(235 + group.members.count * 98)
            let width = min(availableWidth, min(660, max(380, preferredWidth)))
            ...
            let defaultOrigin = NSPoint(x: x, y: top - 38)
            let origin = manualOrigins[group.id] ?? defaultOrigin
            isPositioningProgrammatically = true
            chrome.panel.setFrame(NSRect(origin: origin, size: NSSize(width: width, height: 38)), display: true)
    ...
    func clear() {
        ...
        groupIDByPanel.removeAll()
        manualOrigins.removeAll()
    }
    ...
    private func drop(_ id: String) {
        if let chrome = chromes[id] {
            chrome.panel.orderOut(nil)
            chrome.framePanel.orderOut(nil)
            groupIDByPanel.removeValue(forKey: ObjectIdentifier(chrome.panel))
        }
        chromes[id] = nil
        framedGroupIDs.remove(id)
        anchorWindowIDs.removeValue(forKey: id)
        manualOrigins.removeValue(forKey: id)
    }
    ...
    private func makeChrome(group: LiveTabGroup) -> Chrome {
        ...
        groupIDByPanel[ObjectIdentifier(panel)] = group.id
    ...
    func windowDidMove(_ notification: Notification) {
        guard !isPositioningProgrammatically,
              ...
              let groupID = groupIDByPanel[ObjectIdentifier(panel)]
        else { return }
        manualOrigins[groupID] = panel.frame.origin
    }
