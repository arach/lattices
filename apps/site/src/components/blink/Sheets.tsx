            ...
            {SHEETS.map((s) => {
              ...
              return (
                <button
                  ...
                  tabIndex={on ? 0 : -1}
                  onKeyDown={(event) => {
                    const index = SHEETS.findIndex((sheet) => sheet.id === s.id)
                    const next = event.key === 'Home' ? 0 : event.key === 'End' ? SHEETS.length - 1
                      : ['ArrowRight', 'ArrowDown'].includes(event.key) ? (index + 1) % SHEETS.length
                      : ['ArrowLeft', 'ArrowUp'].includes(event.key) ? (index + SHEETS.length - 1) % SHEETS.length : -1
                    if (next < 0) return
                    event.preventDefault()
                    setActive(SHEETS[next].id)
                    setTouched(true)
                    document.getElementById(`${baseId}-tab-${SHEETS[next].id}`)?.focus()
                  }}
                  onClick={() => {
                    setActive(s.id)
                    setTouched(true)
