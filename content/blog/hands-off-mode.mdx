...
Lattices is open source. The hands-off mode code lives in `bin/handsoff-worker.ts` (the worker), `lib/infer.ts` (the inference wrapper), and `docs/prompts/hands-off-system.md` (the system prompt). The test suite is at `tests/handsoff-tests.ts`.
