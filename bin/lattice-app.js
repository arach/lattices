...
const bundlePath = resolve(appDir, "LatticeApp.app");
const binaryDir = resolve(bundlePath, "Contents/MacOS");
const binaryPath = resolve(binaryDir, "LatticeApp");

const REPO = "arach/lattice";
const ASSET_NAME = "LatticeApp-macos-arm64";

// ── Helpers ──────────────────────────────────────────────────────────

function isRunning() {
  try {
    execSync("pgrep -f LatticeApp.app", { stdio: "pipe" });
    return true;
  } catch {
    return false;
  }
...
function launch(extraArgs = []) {
  if (isRunning()) {
    console.log("lattice app is already running.");
    return;
  }
  const args = [bundlePath];
  if (extraArgs.length) args.push("--args", ...extraArgs);
  spawn("open", args, { detached: true, stdio: "ignore" }).unref();
  console.log("lattice app launched.");
}

// ── Build from source (current arch only) ────────────────────────────

function buildFromSource() {
  console.log("Building lattice app from source...");
  try {
  ...
  const builtPath = resolve(appDir, ".build/release/LatticeApp");
  ...
  try {
    ...
    execSync(
      `codesign --force --sign ${signArg} --identifier com.arach.lattice '${bundlePath}'`,
      { stdio: "pipe" }
    );
  } catch (e) {
...
function httpsGet(url) {
  return new Promise((resolve, reject) => {
    get(url, { headers: { "User-Agent": "lattice" } }, (res) => {
...
async function ensureBinary() {
  ...
  // 3. Nothing worked
  console.error(
    "Could not build or download the lattice app.\n" +
    "Options:\n" +
...
if (cmd === "build") {
  ...
  try {
    execSync("pkill -f LatticeApp.app", { stdio: "pipe" });
    console.log("lattice app stopped.");
  } catch {
    console.log("lattice app is not running.");
  }
} else if (cmd === "restart") {
  // Quit → rebuild → relaunch
  try { execSync("pkill -f LatticeApp.app", { stdio: "pipe" }); } catch {}
  if (!hasSwift()) {
    console.error("Swift is required. Install with: xcode-select --install");
    process.exit(1);
  }
